...
function gotData(incomingData){
  ...
  yAxisGroup.call(yAxis);
    let drawGroup = viz.selectAll("empty").data([incomingData[0]])
    drawGroup.transition().duration(1000);
    let lineGroups = drawGroup.enter().append("g").attr("class", "lineGroup").append("path");
    let lineFunc = d3.line();
    lineFunc.x(function (d, i) {

        return xScale(d.year);
    });
    lineFunc.y(function (d, i) {
        return yScale(d.birthsPerThousand)
    });

     lineGroups
        .transition().attr("d", lineFunc)
    ...
    document.getElementById("usa").addEventListener("click",function(){
        lineGroups.transition().attr("d", lineFunc(incomingData[0])).attr("stroke","blue")
    })
    document.getElementById("china").addEventListener("click",function () {
        lineGroups.transition().attr("d", lineFunc(incomingData[1])).attr("stroke","red")
    })
}
