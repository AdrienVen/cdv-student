...
function gotData(incomingData){
  ...
  let redScale = d3.scaleLinear().domain(popExtent).range([0,255]);
  let blueScale = d3.scaleLinear().domain(fertExtent).range([0,255]);
  let greenScale = d3.scaleLinear().domain(lifeExtent).range([0,255]);


    function colorPicker(d,i){
        let rVal = parseInt(redScale(d.pop))
        let bVal = parseInt(blueScale(d.fert))
        let gVal = parseInt(greenScale(d.life))
  ...
  function drawViz(){
    ...
    console.log("---\nthe currentYearData array now carries the data for year", currentYear);
      let groups = vizGroup.selectAll(".circles").data(currentYearData).enter().append("g")
      let textGroups = vizGroup.selectAll(".labels").data(currentYearData).enter().append("g")
      ...
      vizGroup.selectAll(".circles")
          .attr("r", getRadius)
          .transition().attr("transform",translate)
          .attr("stroke", colorPicker);

      vizGroup.selectAll(".labels").transition().attr("transform",translateText)


        function getRadius(d,i) {
        console.log("get radius called")
            return rScale(d.pop);
        }
        function placeX(d,i){
            return xScale(d.fert)
        }
        function placeY(d,i){
            return yScale(d.life)
        }
        function translate(d,i){
        return 'translate('+placeX(d,i)+','+placeY(d,i)+')'
        }
        function getLabel(d,i){
          return d.Country
        }
      function translateText(d,i){
          return 'translate('+placeX(d,i)+','+(placeY(d,i)+rScale(d.pop)+10)+')'
      }
